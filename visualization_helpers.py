...
#    menu_picos.entryconfig("Peak Function 4", command=lambda:clf_peak(data), state=NORMAL)
...
#    menu_picos.entryconfig("Peak Function 7", command=lambda:lasso_peak(data), state=NORMAL)
...
def _plot_data(data, window, canvas):
    ...
    # Show the scale widget when displaying original data
...
def show_scale_widget(main_window):
    """Show the scale widget in frame2"""
    for widget in main_window.winfo_children():
        if isinstance(widget, tk.Frame):
            for child in widget.winfo_children():
                if isinstance(child, tk.Scale):
                    child.grid()  # Show the widget again
                    break