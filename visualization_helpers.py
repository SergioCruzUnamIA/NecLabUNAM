...
def initialize_visualization(window, menu_picos, canvas):
    ...
    menu_picos.entryconfig("Correlación Pearson", command=lambda:plot_correlation(data,correlation_pearson(data),window,canvas), state=NORMAL)
    menu_picos.entryconfig("Correlación Kendall", command=lambda:plot_correlation(data,correlation_kendall(data),window,canvas), state=NORMAL)
    menu_picos.entryconfig("Correlación Spearman", command=lambda:plot_correlation(data,correlation_spearman(data),window,canvas), state=NORMAL)
    menu_picos.entryconfig("Dendrograma", command=lambda:plot_dendogram(data, window, canvas), state=NORMAL)
